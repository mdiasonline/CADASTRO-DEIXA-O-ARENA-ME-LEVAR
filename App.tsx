...
const App: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("Erro ao gravar no banco. Verifique sua conexão.");
      setSyncStatus('offline');
    } finally {
      setLoading(false);
  ...
  const removeMember = async (id: string) => {
    if (!window.confirm('Deseja excluir permanentemente este registro do banco de dados?')) return;
    ...
    <div className="flex flex-col min-h-screen">
      <header className="bg-[#2B4C7E] text-white py-6 md:py-10 shadow-xl border-b-4 border-[#F9B115] sticky top-0 z-50">
        <div className="container mx-auto px-4 relative">
          <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-2 px-3 py-1 bg-black/20 rounded-full border border-white/10">
            {syncStatus === 'synced' && <><CloudCheck size={16} className="text-green-400" /><span className="text-[10px] font-bold">ONLINE</span></>}
            {syncStatus === 'syncing' && <><CloudDownload size={16} className="text-yellow-400 animate-pulse" /><span className="text-[10px] font-bold">LENDO...</span></>}
            {syncStatus === 'offline' && <><CloudOff size={16} className="text-red-400" /><span className="text-[10px] font-bold">ERRO DB</span></>}
          </div>
          <div className="text-center">
            <h1 className="text-3xl md:text-6xl font-arena tracking-tighter">
        ...
        {view === ViewMode.HOME && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-fadeIn mt-6">
            <button onClick={() => setView(ViewMode.REGISTER)} className="arena-card p-10 group bg-white text-center hover:scale-[1.02] transition-all">
              ...
              <p className="font-bold text-gray-500 uppercase text-xs">Acesso ao Banco de Dados</p>
            </button>
            <button onClick={() => setView(ViewMode.LIST)} className="arena-card p-10 group bg-white text-center hover:scale-[1.02] transition-all">
              ...
              <p className="font-bold text-gray-500 uppercase text-xs">Registros Sincronizados</p>
            </button>
          </div>
        )}
          ...
          <div className="max-w-md mx-auto arena-card overflow-hidden bg-white animate-slideUp">
            ...
            {isRegistered ? (
              <div className="p-10 text-center">
                ...
                <h3 className="text-2xl font-arena text-[#2B4C7E] mb-2">GRAVADO COM SUCESSO!</h3>
                <p className="text-sm font-bold text-gray-400 mb-8 uppercase tracking-widest">Registro enviado para o servidor</p>
              ...
              <form onSubmit={handleSubmit} className="p-8 space-y-5">
                ...
                <button type="submit" disabled={loading} className="btn-arena w-full py-4 rounded-2xl font-arena text-2xl flex items-center justify-center gap-3">
                  {loading ? <Loader2 className="animate-spin" /> : "GRAVAR NO BANCO"}
                </button>
              </form>
            )}
          </div>
          ...
          <div className="space-y-6 animate-fadeIn">
            <div className="flex flex-col md:flex-row gap-4">
              <div className="flex-grow bg-white p-3 rounded-2xl border-4 border-[#2B4C7E] flex items-center gap-3 shadow-sm">
                ...
                <input placeholder="PESQUISAR NO BANCO..." className="w-full outline-none font-bold text-[#2B4C7E]" onChange={e => setSearchTerm(e.target.value)} />
              </div>
            ...
            {fetching ? (
              <div className="flex flex-col items-center justify-center py-20 gap-4">
                ...
                <p className="font-arena text-xl text-[#2B4C7E] animate-pulse">SINCRONIZANDO DADOS...</p>
              </div>
            ) : (
